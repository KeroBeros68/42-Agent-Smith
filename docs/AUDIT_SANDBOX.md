...
Aucun point ouvert — voir « Corrigés depuis l'audit initial ».
...
Les 6 contraintes de sécurité §V.2.3, le relais `tool_call`/`final_answer`, et les deux transports MCP (stdio + HTTP) sont maintenant couverts et vérifiés en conditions réelles Docker. **Aucune priorité ouverte propre à `student/sandbox/` au 2026-09-06** — le module est fonctionnellement complet et testé de bout en bout sur toute sa surface documentée ici, y compris le dernier point trouvé et corrigé (`session.py::build_container()`, voir « Corrigés »). `AUDIT_AGENT_CORE.md` et `AUDIT_SWEBENCH_TOOLS.md` sont également sans priorité ouverte à cette date.
...
| **`sandbox/session.py::build_container()` supposait silencieusement `inputSchema.properties`** (même schéma de bug que `agent_core/manual.py`, corrigé plus tôt le même jour — voir `AUDIT_AGENT_CORE.md`, Corrigés) — mais conséquence plus grave ici : `properties.keys()` alimente `runner.py`'s `_make_tool_stub()` (`zip(param_names, args)`), donc un tool au schéma non-standard aurait silencieusement perdu **tous** ses arguments positionnels à l'exécution, sans erreur visible avant l'appel lui-même | `sandbox/session.py` (`build_container()` lève désormais `ValueError` explicite si `"properties" not in schema`, au lieu de `.get("properties", {})` silencieux) | Choix délibéré d'échouer tôt et bruyamment (au démarrage de session, quand les tools sont énumérés) plutôt que de dégrader silencieusement comme `manual.py` — la conséquence en aval (perte d'arguments) est plus dure à diagnostiquer qu'une description de tool trompeuse. Vérifié : schéma standard inchangé (aucune régression), schéma `$ref` synthétique lève bien `ValueError` avec un message clair, absence de `mcp_bridge` inchangée (`tools = {}`). **Test réel avec les 9 vrais tools SWE-bench** (schémas réels via `mcp_tools_swebench.py`) : tous passent sans lever d'exception, confirmant 0 faux positif. Non-régression pipeline complet (`make run BENCH=mbpp`) : `success: true`. `mypy`/`flake8` propres | 2026-09-06 |
| `repl.py::_format_response` dupliquait en substance `agent_core/loop.py::_format_observation` (même classification `result`/`error`/`final_answer`/fallback) — en déduppliquant, une incohérence trouvée *dans* `repl.py` lui-même : le cas `error` n'ajoutait un `\n` final que si `traceback` était absent, jamais quand il était présent, contrairement à `result`/`final_answer`/fallback qui en ont toujours un | `executor/protocol.py` (nouvelle fonction `response_text()`, la classification partagée sans habillage), `repl.py` (`_format_response` devient un fin wrapper au-dessus, ne gère plus que son propre préfixe `final_answer: `/`\n` humain) | Détail complet, comparaison ancien/nouveau sur 5 cas et test en conditions Docker réelles (3 vrais appels `exec` : erreur, résultat, `final_answer`) dans `AUDIT_AGENT_CORE.md` (Corrigés, `loop.py`) — seul changement de sortie réel : le `\n` manquant sur `error` avec traceback, désormais cohérent avec les autres cas | 2026-09-06 |
| **`dir` absent de `SAFE_BUILTINS`** — fonction d'introspection standard, sans risque au-delà de ce que le module documente déjà comme accepté (contournement par `__subclasses__()`). Trouvé par l'exam : `test_dynamic_discovery.py`/`test_sandbox_manual.py` utilisent `if tool in dir():` pour vérifier qu'un outil MCP est bien exposé dans le namespace, et recevaient `NameError: name 'dir' is not defined` — la découverte d'outils rapportait donc 0/3 | `student/sandbox/executor/restrictions.py` | `dir` ajouté à l'allowlist, à sa place alphabétique | Vérifié en sandbox réel : `"add" in dir()` → `True` avec un vrai serveur MCP connecté ; les tests `dynamic_discovery` et `sandbox_manual` de `exam_sandbox.sh` passent (échouaient avant) | 2026-09-09 |
| **Le REPL cassait sur tout fichier multi-lignes pipé** (`cat script.py | uv run sandbox`). `_read_block()` utilise `codeop.compile_command(..., "single")`, qui émule la saisie interactive humaine où une ligne vide signale la fin d'un bloc. Or un fichier Python normal contient des lignes vides *à l'intérieur* d'un bloc encore ouvert (simple lisibilité) : le REPL les prenait pour une fin de bloc, exécutait un fragment tronqué, puis la ligne suivante — indentée car censée être imbriquée — devenait un statement invalide, d'où une cascade d'`IndentationError`. Le vrai agent n'est jamais touché (il envoie toujours un bloc complet), mais `uv run sandbox` en mode pipé l'était pour tout script à structure imbriquée | `student/sandbox/repl.py` (`run()`) | Un stdin non-interactif (`sys.stdin.isatty()` faux) est lu en entier et exécuté comme **un seul** bloc, à la manière de `python -` sur un script pipé ; le mode interactif (TTY réel) reste inchangé, protégé par un early-return | Vérifié réel : `test_dynamic_discovery.py` pipé → `=== DYNAMIC DISCOVERY OK ===`, 3/3 outils découverts et appelés (cascade d'`IndentationError` avant) ; `exam_sandbox.sh` 14/14 | 2026-09-09 |
| **Un dépassement mémoire tuait tout le conteneur au lieu de lever un `MemoryError` rattrapable.** Seule une limite dure côté Docker existait (`mem_limit`, cgroup) : une allocation de 512 Mo sous une limite de 256 Mo faisait tuer le conteneur par le kernel (`Connection to container lost`), sans qu'aucun code sandboxé puisse l'observer ou s'en remettre | `student/sandbox/executor/runner.py` (`main()`) | Limite **molle** en-process via `resource.setrlimit(RLIMIT_AS, ...)` à 90 % de `max_memory_mb` : l'allocateur Python échoue de lui-même avant que le cgroup n'intervienne, la limite Docker restant le filet dur | Vérifié réel avec `sandbox_config_resources.json` (256 Mo) : petite allocation (1 Mo) → OK, grande (512 Mo) → `MemoryError` rattrapé proprement, `=== MEMORY TEST COMPLETE ===` ; le test `memory` de `exam_sandbox.sh` passe (échouait avant) | 2026-09-09 |
| **Aucun mécanisme ne récupérait les conteneurs laissés par une session tuée en SIGKILL.** `with container:` nettoie à toute sortie normale et même sur SIGTERM (`agent_core/shutdown.py` le convertit en exception), mais un SIGKILL ne peut pas être intercepté — le conteneur reste alors *running* indéfiniment. Constaté réellement : 3 orphelins accumulés sur 3 runs d'exam, un par run, laissés par le test bonus « Layer 1 » qui tue volontairement le processus (le `timeout` frappe le wrapper `bash`, qui ne relaie pas au process) | `student/sandbox/container.py` (`_sweep_orphaned_containers()`, `_pid_alive()`, appelées en tête de `start()`) | Balayage au démarrage de chaque session : les conteneurs portant le label `agent-smith.owner-pid` dont le PID propriétaire est mort (`os.kill(pid, 0)`) sont supprimés en `force=True`. Un PID recyclé fait sauter un orphelin — conservateur, un run ultérieur le balaiera. Les **images** sont volontairement épargnées : le tag de l'image dérivée est partagé entre sessions, le supprimer ici casserait un run concurrent. C'est le seul lecteur du label, désormais purement local (le couplage inter-processus qu'il servait a disparu avec le découplage du serveur MCP, voir `AUDIT_SWEBENCH_TOOLS.md`) | Vérifié réel avec discrimination des deux cas : un faux orphelin (owner PID mort) **et** un témoin labellé avec un PID durablement vivant, créés côte à côte → après une session sandbox, l'orphelin est supprimé et le témoin intact (propriété de sûreté essentielle : une session concurrente n'est jamais touchée). Premier essai du test invalidé par erreur de ma part — j'avais pris `$$` comme PID « vivant », or chaque appel shell est un process distinct, donc déjà mort : le balayage avait raison de le supprimer | 2026-09-09 |
